...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(
        ...
        // 2. 인증방식
//        http.formLogin(Customizer.withDefaults()); // 스프링 제공하는 기본 로그인폼
        http.formLogin(formConfig
                -> formConfig
                .loginPage("/user/signin") // GET: CORS
                .loginProcessingUrl("/user/signin")// 로그인폼에서 보낸 post요청을 처리할 url. POST: CSRF
                // 세션에 리턴url이 없다면 여기로
                //.successForwardUrl("/") // post요청
                .defaultSuccessUrl("/")); // get요청
//                .successHandler())
